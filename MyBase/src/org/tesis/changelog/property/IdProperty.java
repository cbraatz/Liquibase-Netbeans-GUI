 ...
 * @author Claus
 */
public final class IdProperty extends Property{
    public IdProperty(String value) throws InvalidValueException {
        this.setKey();
        this.setValue(value);
    }
    ...
    public boolean validateValue(String value) throws InvalidValueException{
        if(!PropertyValueValidator.validateNotEmptyString(value)){
            throw new InvalidValueException("Valor '"+value+"' inválido para la propiedad '"+Constants.PROPERTY_ID+"'. ");
        }else{
            return true;
        }
    }
    ...
    @Override
    public void setKey(){
        this.key=Constants.PROPERTY_ID;
    }
}
